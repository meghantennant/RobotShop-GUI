...
Sales_Associate_Dialog *sales_associate_dialog;
...
void show_modelCB(Fl_Widget *w, void *p)
{
			...
			Fl_Help_View *out = new Fl_Help_View(30, 30, 210, 150, "Robot Model");
			out->value(model);

			Fl_Box *model_image = new Fl_Box(150, 200, 50, 50);
			Fl_JPEG_Image *m_image = new Fl_JPEG_Image("full_humanoid_meditating.jpg");
			model_image->image(m_image);
...
void create_customerCB(Fl_Widget *w, void *p)
{
	...
	string email = customer_dialog->email();
	...
	shop.add_customer(name, customer_number, email, wallet);
}

void SalesCB(Fl_Widget *w, void *p) { sales_associate_dialog->show(); }
void cancel_sales_associateCB(Fl_Widget *w, void *p) { sales_associate_dialog->hide(); }
void create_sales_associateCB(Fl_Widget *w, void *p)
{
	//Converts dialog input into variables

	string name = sales_associate_dialog->name();
	string employee_number = sales_associate_dialog->employee_number();

	shop.add_sales_associate(name, employee_number);
}

void OrderCB(Fl_Widget *w, void *p) { 
	if (shop.robotModels == NULL)
	{
		fl_message("Need to add a robot model!");
		robot_model_dialog->show();
	}
	if (shop.customers == NULL)
	{
		fl_message("There are no customers in the system!");
		customer_dialog->show();
	}
	if (shop.sales_associates == NULL)
	{
		fl_message("There are no sales associates in the system!");
		customer_dialog->show();
	}
	order_dialog->show();
}
void cancel_orderCB(Fl_Widget *w, void *p) { order_dialog->hide(); }
void create_orderCB(Fl_Widget *w, void *p)
{
	//Converts dialog input into variables
	string order_number = order_dialog->number();
	string model = order_dialog->model();
	string customer = order_dialog->customer();
	string sales_associate = order_dialog->sales_associate();

	shop.add_order(order_number, model, customer, sales_associate);
}
...
Fl_Menu_Item menuitems[] = {
	...
	{"&Create", 0, 0, 0, FL_SUBMENU},
		...
		{"&Sales Associate",0,(Fl_Callback *)SalesCB},
...
int main() {
	...
	sales_associate_dialog = new Sales_Associate_Dialog{};
	order_dialog = new Order_Dialog{};
